...
export interface CoverflowCarouselProps {
  ...
  /** Called whenever the centred slide changes (drag, arrows, dots, or a card click). */
          ...
          >
            {slides.map((slide, index) => (
              <div
                ...
                onClick={() => goTo(index)}
                className={cn(
                  "absolute left-1/2 top-0 aspect-square will-change-transform cursor-pointer",
                  cardClassName,
                )}
                style={{ width: "var(--cf-card)" }}
              >
                {selected === index && (
                  <span className="absolute -top-7 left-1/2 -translate-x-1/2 whitespace-nowrap text-xs font-semibold text-foreground bg-card border border-border/50 rounded-full px-3 py-1 shadow-sm z-20 duration-200 animate-in fade-in slide-in-from-bottom-1">
                    {slide.alt}
                  </span>
                )}
                <div className="h-full w-full overflow-hidden rounded-2xl bg-muted shadow-xl">
