      ...
      <CardHeader className="p-0">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            {onToggle && (
              <Button className="size-8" variant="ghost" size="icon" onClick={onToggle}>
                {isOpen ? (
                  <PanelRightClose className="size-4.5 text-muted-foreground" strokeWidth={1.5} />
                ) : (
                  <PanelRightOpen className="size-4.5 text-muted-foreground" strokeWidth={1.5} />
                )}
              </Button>
            )}
          </div>
        ...
        <div
          className={`overflow-hidden transition-all duration-300 ease-in-out ${
            isOpen ? "max-h-12 opacity-100 mt-4" : "max-h-0 opacity-0 mt-0"
          }`}
        >
          <Tabs
            value={activeTab}
